...
directoryForDB = "D:/Workshop/homework/Data Analytics/Lab2/"
...
with con:
	...
	pop = con.cursor()
	pop.execute("DROP TABLE IF EXISTS popANDdensity") 
	pop.execute("CREATE TABLE popANDdensity(neighborhood TEXT, pop1940 INT, pop1950 INT, pop1960 INT, pop1970 INT, pop1980 INT, pop1990 INT, pop2000 INT, pop2010 INT)")
	for key in pop1940:
...
with con:
	emp = con.cursor()
	emp.execute("DROP TABLE IF EXISTS employment") 
	emp.execute("CREATE TABLE employment(neighborhood TEXT,jobConstruction REAL,jobManufacturing REAL,jobRetail REAL,jobTransportUtilities REAL,jobInformation REAL,jobFinance REAL,jobScientific REAL,jobEduHealthSoc REAL,jobArtsRecreation REAL,jobPublicAdmin REAL,jobOther REAL)")
