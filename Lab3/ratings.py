...
for line in response: #  store names and ratings into movies(dict)
	...
	#count = count +1
	#print count
	if(name[0] in moviesvote.keys()):
	    if(vote>moviesvote[name[0]]): #  If current vote is more than same-name movie in moviesvote(dict), update vote and rate
	        moviesvote[name[0]] = vote
	        movies[name[0]] = float(save[2].replace("'", ""))
	        print name[0]
	        print vote
	else: #  If current movie doesn't exisit in moviesvote(dict), add it in.
	    moviesvote[name[0]] = vote
	    movies[name[0]] = float(save[2].replace("'", ""))
	    print name[0]
	    print vote
