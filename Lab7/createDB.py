...
with con:# Create database
	...
	pop.execute("CREATE TABLE node(nodeid TEXT, PRIMARY KEY(nodeid))")# Set nodeid as Primary key to avoid dupicate
	pop.execute("DROP TABLE IF EXISTS link") 
	pop.execute("CREATE TABLE link(nodeid TEXT, linkto TEXT, PRIMARY KEY(nodeid, linkto))")# Set nodeid, linkto as Primary key, one link appears only once
	con.commit()
